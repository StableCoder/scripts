...
try {   
    ...
    if($null -eq ( ";C:\\sdl3\\bin" | ? { [System.Environment]::GetEnvironmentVariable("PATH","Machine") -match $_ })) {
        # PATH
        [Environment]::SetEnvironmentVariable( "PATH", [System.Environment]::GetEnvironmentVariable("PATH","Machine") + ";C:\sdl3\bin", [System.EnvironmentVariableTarget]::Machine )
    }
} catch {
    # Delete our working directory
    cd $invocationDir
